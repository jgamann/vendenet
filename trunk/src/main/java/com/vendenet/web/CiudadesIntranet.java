...
import com.vendenet.negocio.negocio.NegoCiudadesIntranet;
 ...
 * Ventana&gt;Preferencias&gt;Java&gt;Generaci�n de c�digo&gt;C�digo y comentarios
 */
public class CiudadesIntranet implements ProcesoWebPadre {
	private Logger logger = Logger.getLogger(CiudadesIntranet.class);
	private CiudadesIntranet inicio=null;
	...
	public CiudadesIntranet(){
	}
	
	public CiudadesIntranet(HttpServletRequest req) {
		Session session=HibernateUtil.getSessionFactory().openSession();
		Transaction transaction = null;
		try{
			HttpSession sesion = req.getSession();
			NegoCiudadesIntranet negoCiudadesIntranet = new NegoCiudadesIntranet();
			...
			if(usuario!=null){
				negoCiudadesIntranet.recogerCiudades(session);
				hsResultados = negoCiudadesIntranet.getHsResultados();
				sesion.setAttribute(TextConstant.KEY_USUARIO, usuario);
				strPaginaDestino="jsp/ciudades_intranet.jsp";
	...
	{
		inicio = new CiudadesIntranet(req);
		return inicio;
	}
