...
import com.vendenet.negocio.negocio.NegoPonerAnuncio;
 ...
 * Ventana&gt;Preferencias&gt;Java&gt;Generación de código&gt;Código y comentarios
 */
public class PonerAnuncio implements ProcesoWebPadre {
	private Logger logger = Logger.getLogger(PonerAnuncio.class);
	private PonerAnuncio inicio=null;
	...
	public PonerAnuncio(HttpServletRequest req) {
		Session session=HibernateUtil.getSessionFactory().openSession();
		Transaction transaction = null;
		try{
			NegoPonerAnuncio negoPonerAnuncio = new NegoPonerAnuncio();
			...
			negoPonerAnuncio.recogerContenido(session);
			strPaginaDestino="jsp/poner_anuncio.jsp";
			transaction.commit();
			session.close();
			hsResultados = negoPonerAnuncio.getHsResultados();
