...
import com.vendenet.negocio.negocio.NegoHistoricoEmails;
 ...
 * Ventana&gt;Preferencias&gt;Java&gt;Generaci�n de c�digo&gt;C�digo y comentarios
 */
public class HistoricoEmails implements ProcesoWebPadre {
	private Logger logger = Logger.getLogger(HistoricoEmails.class);
	private HistoricoEmails inicio=null;
	...
	public HistoricoEmails(HttpServletRequest req) {
		Session session=HibernateUtil.getSessionFactory().openSession();
		Transaction transaction = null;
		try{
			HttpSession sesion = req.getSession();
			NegoHistoricoEmails negoHistoricoEmails = new NegoHistoricoEmails();
			...
			if(usuario!=null){
				negoHistoricoEmails.recogerHistorico(session,fechainicio,fechafin);
				hsResultados = negoHistoricoEmails.getHsResultados();
				sesion.setAttribute(TextConstant.KEY_USUARIO, usuario);
				strPaginaDestino="jsp/historico_emails_intranet.jsp";
	...
	{
		inicio = new HistoricoEmails(req);
		return inicio;
	}
